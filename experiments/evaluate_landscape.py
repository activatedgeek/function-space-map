...
def perturb_params(key, params, std=1.0, step_lim=1.0, n_steps=10):
    ## Sample a direction.
    flat_p, tree_unflatten_fn = jax.flatten_util.ravel_pytree(params)
    raw_direction = tree_unflatten_fn(
        std * jax.random.normal(key, shape=flat_p.shape, dtype=flat_p.dtype)
    )
    ...
    ## Steps in direction.
    step_sizes = jnp.linspace(
        -step_lim, step_lim, n_steps + ((n_steps + 1) % 2)
    )  ## additional 1 gets us 0. when n_steps is even, i.e. no perturbation.
    step_params = jax.vmap(
        lambda alpha: jax.tree_util.tree_map(
...
def compute_loss_fn(model, batch_params, batch_extra_vars):
    ...
    pmap_model_fn = jax.pmap(
        jax.vmap(model_fn, in_axes=(0, 0, None)), in_axes=(0, 0, None)
    )
...
def compute_mutables_fn(model, batch_params, extra_vars, update_mutables=True):
    @jax.jit
...
def main(
    ...
    train_data, *_ = get_dataset(
        dataset, augment=False, root=data_dir, seed=seed, channels_last=True
    )
    train_loader = get_loader(train_data, batch_size=batch_size)

    model, params, extra_vars = get_model(
        model_name,
        model_dir=model_dir,
    ...
    batch_params = jax.pmap(
        lambda _k: perturb_params(_k, params, step_lim=step_lim, n_steps=n_steps)
    )(jnp.array(directions_rng))
    batch_extra_vars = compute_mutables_fn(
        model, batch_params, extra_vars, update_mutables=bool(update_mutables)
    )(train_loader)
    rnd_directions_loss = compute_loss_fn(model, batch_params, batch_extra_vars)(
        train_loader
    )

    if jax.process_index() == 0:
        with open(Path(log_dir) / f"results.npz", "wb") as f:
            jnp.savez(f, seed=seed, step_lim=step_lim, losses=rnd_directions_loss)
