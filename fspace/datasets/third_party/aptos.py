...
def get_aptos_orig(
    ...
    **_
    ...
    for i in range(n_classes):
        ...
        validation_idx.extend(
            np.random.choice(
                label_idx, int(val_size * len(label_idx)), replace=False
            ).tolist()
        )
    ...
    testset = AptosDataset(
        df=validation_split_df, data_path=train_path, transform=transform
    )
    ...
    trainset = AptosDataset(
        df=train_split_df, data_path=train_path, transform=transform_train
    )
