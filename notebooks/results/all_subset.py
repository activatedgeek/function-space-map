...
cols = ['run_id', 'dataset', 'method', 'train_subset', 'seed',
        's/test/acc', 's/test/sel_acc', 's/test/avg_nll', 's/test/ece', 's/ood_test/auc']
__mpsmap = pd.concat([_m1, _m2], ignore_index=True)[cols].reset_index().rename(
    columns={ 's/test/acc': 'acc', 's/test/sel_acc': 'sel_acc', 's/test/avg_nll': 'nll', 's/test/ece': 'ece', 's/ood_test/auc': 'ood_auroc' })

__mpsmap['acc'] *= 100.
__mpsmap['ece'] *= 100.
__mpsmap['ood_auroc'] *= 100.
...
__mfsgc = pd.concat([_m3, _m4], ignore_index=True)[cols].reset_index().rename(
...
metrics = pd.concat([__mpsmap, __mfsgc], ignore_index=True).reset_index().drop(columns=['level_0', 'index'])
# metrics.to_csv('results/all_subset.csv', index=False)

mu = metrics.groupby(['dataset', 'train_subset', 'method']).mean(numeric_only=True).drop(columns=['seed'])
...
sigma = metrics.groupby(['dataset', 'train_subset', 'method']).std(numeric_only=True).drop(columns=['seed'])
